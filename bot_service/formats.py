...
def format_filters_human_readable(filters: dict) -> str:
    ...
    if filters.get('job_types'):
        job_types = ', '.join(map(lambda x: {
            'full_time': 'تمام‌وقت',
            'part_time': 'پاره‌وقت',
            'contract': 'پروژه‌ای',
            'internship': 'کارآموزی'
        }.get(x, x), filters['job_types']))
        lines.append(f"🧩 نوع همکاری: {job_types}")

    if filters.get('seniorities'):
        levels = ', '.join(map(lambda x: {
            'junior': 'جونیور',
            'mid': 'میدل',
            'senior': 'سینیور'
        }.get(x, x), filters['seniorities']))
    ...
    if filters.get('salary_type'):
        ...
        lines.append(f"💰 نوع حقوق: {types.get(filters['salary_type'], filters['salary_type'])}")
...
def format_job(job: dict) -> str:
    ...
    seniority = ', '.join({
        ...
        'mid': 'میدل',
        'senior': 'سینیور'
    }.get(level, level) for level in job.get('seniority_level', [])) or 'نامشخص'
    ...
    salary_type = {
        'negotiable': 'توافقی',
        'fixed': 'ثابت'
    }.get(job.get('salary_type'), 'نامشخص')
    ...
    if salary_type == 'توافقی' and not base_salary:
        ...
        salary_display = f"حقوق پایه وزارت کار"
    else:
    ...
    description_summary = description[:200].strip() + ('...' if len(description) > 200 else '')

    url = job.get('url', 'ندارد')
    source = job.get('source', 'نامشخص')
    military_status = ', '.join(job.get('military_status', [])) if job.get('military_status') else 'مهم نیست'
